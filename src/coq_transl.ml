...
(* canonical representation using var. renaming starting at n along with variable substitutions *)
let rec can_aux n t =
  let f = can_aux n in
    match t with
    | Var x                 -> Var x
...
and remove_lambda ctx tm =
  ...
  let ctx' = get_fvars ctx tm in
    let (cctx,sigma,ctm) = canonical ctx' tm in
      let revsigma = List.map (fun (x,y) -> (y,x)) sigma in
        try
          let hashtm = Hashtbl.find funcs_hash (cctx,ctm) in
